...
_По результатам проверки задания выполнена доработка результирующей БД ([описание процесса](update1.md))_
...
Для соблюдения требований к нормализации вынесем элементы массива *order_list* в таблице *orders* в отдельную таблицу, одновременно преобразовав их к формату `uuid`.
```sql
select 
   order_pk, 
...
from orders
```
И удалим *order_list* из таблицы *orders*.
```sql
alter table orders 
drop column order_list
```

Для надежности снова проверим, что можем собрать исходный набор данных с помощью JOIN-ов
```sql
select count(*) 
from (
	select 
...
### 6.
В завершение добавим к таблицам ограничения на первичные и внешние ключи.
```sql
